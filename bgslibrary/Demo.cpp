...
int main(int argc, char **argv)
{
  ...
  while(key != 'q')
  {
    ...
    GaussianBlur( img_input, img_input, cv::Size(11,11), 0 );
    ...
    int rows = img_mask.rows;
    int cols = img_mask.cols;
    ...
    //cv::Mat des;
    //cv::bitwise_and(img_input,des, img_mask = img_mask);
    ...
    //cv::Mat result;
    //cv::subtract(img_input, img_mask, result);

    //cv::imshow("asd", result);
    //cv::imshow("asd2", img_mask);
    //if(!img_mask.empty())
