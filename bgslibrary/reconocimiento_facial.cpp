...
CascadeClassifier profileFaceDetector;
...
double DESIRED_LEFT_EYE_Y = 0.04;
double DESIRED_LEFT_EYE_X = 0.09;  

int FaceWidth = 50;
int FaceHeight = 50;

int REDUCED_SIZE = 1000;
...
bool GetFaceAndEyes(Mat& frame, Rect& rostro, Rect& lEye, Rect& rEye)
{
	...
	profileFaceDetector.detectMultiScale(frame, faces, 1.1, 3, CASCADE_FIND_BIGGEST_OBJECT | CASCADE_DO_ROUGH_SEARCH);
...
bool Init()
{
	if(!capture.open("dataset/denis.avi"))
	//if(!capture.open(0))
	{
	...
	if(!faceDetector.load("/usr/share/opencv/haarcascades/haarcascade_frontalface_alt.xml"))
	{
	...
	if(!lEyeDetector.load("/usr/share/opencv/haarcascades/haarcascade_eye_tree_eyeglasses.xml"))
	{
		cout << "No se encuentra el archivo haarcascade_eye_tree_eyeglasses.xml 1" << endl;
		return false;
	}

	if(!rEyeDetector.load("/usr/share/opencv/haarcascades/haarcascade_eye_tree_eyeglasses.xml"))
	{
...
int main()
{
	...
	while (correct)
	{
		...
		if(GetFaceAndEyes(copyFrame, face, lEye, rEye))
		{
			...
			{
				Mat nface;
				CropFace(copyFrame, nface, lEye, rEye);
